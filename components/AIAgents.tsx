...
import { Tab, Agent as AgentType } from '../types.ts';
import { GoogleGenAI, Chat } from '@google/genai';
import { Send, Sparkles, Trash2, Bot } from './Icons.tsx';
...
const AIAgents: React.FC<AIAgentsProps> = ({ setActiveTab }) => {
  ...
  const initializeChat = useCallback((agent: AgentType, historyToRestore?: ChatMessage[]) => {
    try {
  ...
  const switchAgent = useCallback((agent: AgentType) => {
    ...
    initializeChat(agent, initialHistory);
  ...
  const handleClearChat = () => {
    ...
    initializeChat(activeAgent, freshHistory);
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[85vh]">
      ...
      <div className="bg-brand-glass border border-slate-700/50 rounded-xl p-4 flex flex-col animate-slide-in-up">
        <h2 className="text-xl font-bold mb-4 text-center">Your AI Team</h2>
