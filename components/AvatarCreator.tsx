...
import { Star, RefreshCw, User as UserIcon, Download, Mic, Phone, MessageSquare, Send, UploadCloud, Wand, Edit, ChevronDown, Image, X } from './Icons.tsx';
            ...
            {phase === 'design' && (
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    <div className="lg:col-span-3 bg-brand-glass border border-slate-700/50 rounded-2xl p-6 space-y-5">
                        <div className="segmented-control"><button onClick={() => setGenerationMode('scratch')} className={`segmented-control-button ${generationMode === 'scratch' ? 'active' : ''}`}>From Scratch</button><button onClick={() => setGenerationMode('photo')} className={`segmented-control-button ${generationMode === 'photo' ? 'active' : ''}`}>From Photo</button></div>
                        {error && <p className="text-red-400 text-center text-sm bg-red-500/10 p-2 rounded-lg">{error}</p>}
                        {generationMode === 'photo' && (
                            <label onDrop={handleDrop} onDragOver={handleDragEvents} onDragEnter={handleDragEvents} onDragLeave={handleDragEvents} className={`flex flex-col items-center justify-center w-full h-48 border-2 border-slate-700 border-dashed rounded-xl cursor-pointer bg-slate-800/50 transition-colors ${isDragging ? 'border-violet-500 bg-slate-700/50' : 'hover:bg-slate-700/50'}`}>
                                {sourceImageBase64 ? <img src={sourceImageBase64.url} alt="Source" className="w-full h-full object-contain rounded-lg p-2" /> : <><UploadCloud className="w-8 h-8 mb-4 text-slate-400" /><p className="mb-2 text-sm text-slate-400"><span className="font-semibold">Click to upload</span> or drag and drop</p><p className="text-xs text-slate-500">PNG or JPG (MAX. 4MB)</p></>}
                                <input id="photo-upload" type="file" className="hidden" accept="image/png, image/jpeg" onChange={handleFileChange} />
                            </label>
                        )}
                        <div className="space-y-4">
                            <h3 className="font-bold text-slate-100">Appearance</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><CustomSelect label="Gender" value={gender} onChange={setGender} options={genders} /><CustomSelect label="Avatar Style" value={avatarStyle} onChange={setAvatarStyle} options={avatarStyles} /><CustomSelect label="Hair Style" value={hairStyle} onChange={setHairStyle} options={hairStyles} /><div><label className="input-label">Hair Color</label><input type="text" value={hairColor} onChange={e => setHairColor(e.target.value)} placeholder="e.g., Electric Blue" className="form-input"/></div><CustomSelect label="Eye Color" value={eyeColor} onChange={setEyeColor} options={eyeColors} /><CustomSelect label="Facial Hair" value={facialHair} onChange={setFacialHair} options={facialHairOptions} /><CustomSelect label="Glasses" value={glasses} onChange={setGlasses} options={glassesOptions} /><div><label className="input-label">Other Features</label><input type="text" value={otherFacialFeatures} onChange={e => setOtherFacialFeatures(e.target.value)} placeholder="e.g., Freckles" className="form-input"/></div></div>
                            <h3 className="font-bold text-slate-100 pt-2">Clothing & Scene</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="input-label">Top</label><input type="text" value={clothingTop} onChange={e => setClothingTop(e.target.value)} placeholder="e.g., Leather Jacket" className="form-input"/></div><div><label className="input-label">Hat</label><input type="text" value={accessoriesHat} onChange={e => setAccessoriesHat(e.target.value)} placeholder="e.g., Beanie" className="form-input"/></div><div className="sm:col-span-2"><label className="input-label">Background</label><input type="text" value={background} onChange={e => setBackground(e.target.value)} placeholder="e.g., Neon-lit alleyway" className="form-input"/></div><CustomSelect label="Shot Type" value={shotType} onChange={setShotType} options={shotTypes} /><div className="sm:col-span-2"><label className="input-label">Personality (1 sentence)</label><input type="text" value={personality} onChange={e => setPersonality(e.target.value)} placeholder="e.g., A cynical detective..." className="form-input"/></div></div>
                        </div>
                    ...
                    <div className="lg:col-span-2 bg-black/20 border border-slate-700/50 rounded-2xl p-6 flex flex-col items-center justify-center"><div className="w-full max-w-md aspect-square bg-slate-900/30 rounded-lg flex items-center justify-center relative overflow-hidden"><UserIcon className="w-24 h-24 mx-auto text-slate-700 animate-breathing" /><p className="absolute bottom-6 font-semibold text-slate-500">Your avatar will appear here</p></div></div>
                </div>
            )}
            
            {phase === 'generating' && (
                <div className="text-center py-10 bg-brand-glass border border-slate-700/50 rounded-xl p-6 shadow-xl"><svg viewBox="0 0 200 200" className="w-48 h-48 mx-auto text-violet-400 opacity-60"><g fill="none" stroke="currentColor" strokeWidth="1"><path className="neural-line" style={{animationDelay: '0s'}} d="M20 100 S 50 20, 100 50 S 150 180, 180 100"/><path className="neural-line" style={{animationDelay: '-1s'}} d="M20 100 S 50 180, 100 150 S 150 20, 180 100"/><path className="neural-line" style={{animationDelay: '-2s'}} d="M20 100 S 80 40, 100 100 S 120 160, 180 100"/><path className="neural-line" style={{animationDelay: '-3s'}} d="M20 100 S 80 160, 100 100 S 120 40, 180 100"/><circle className="neural-point" cx="20" cy="100" r="2" style={{animationDelay: '0s'}}/><circle className="neural-point" cx="180" cy="100" r="2" style={{animationDelay: '0s'}}/><circle className="neural-point" cx="100" cy="50" r="2" style={{animationDelay: '-0.5s'}}/><circle className="neural-point" cx="100" cy="150" r="2" style={{animationDelay: '-0.5s'}}/><circle className="neural-point" cx="100" cy="100" r="2" style={{animationDelay: '-1s'}}/></g></svg><p className="mt-4 text-slate-300 font-semibold text-lg animate-text-fade-cycle">Generating Your Masterpiece...</p><p className="text-sm text-slate-400">This can take a moment.</p></div>
            )}

            {phase === 'interact' && avatarBase64 && (
                 <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                    <div className="lg:col-span-2 space-y-6"><div className="bg-brand-glass border border-slate-700/50 rounded-2xl p-4"><div className="relative"><img src={`data:image/png;base64,${avatarBase64}`} alt="Generated Avatar" className="w-full aspect-square object-contain rounded-lg bg-slate-900/30 animate-avatar-breathing" />{interactionMode === 'voice' && <div className="absolute bottom-4 left-1/2 -translate-x-1/2"><StatusIndicator status={status} /></div>}</div><div className="mt-4 flex flex-col gap-3"><button onClick={handleDownloadAvatar} className="button-secondary w-full" title="Download the generated avatar image as a PNG file"><Download className="w-4 h-4 mr-2"/> Download</button><button onClick={handleStartOver} className="button-secondary w-full" title="Go back to the design screen to create a new avatar"><RefreshCw className="w-4 h-4 mr-2"/> Create New</button></div></div><div className="bg-brand-glass border border-slate-700/50 rounded-2xl p-4"><h3 className="font-semibold text-slate-200 mb-2">Quick Edit</h3><div className="flex gap-2"><input type="text" value={editPrompt} onChange={e => setEditPrompt(e.target.value)} placeholder="e.g., make the hair blue" className="form-input" title="Describe a change, e.g., 'make the hair blue' or 'add sunglasses'"/><button onClick={handleApplyEdit} disabled={isEditingAvatar} className="button-primary px-3" title="Apply the edit instruction to the current avatar">{isEditingAvatar ? <Spinner size="sm"/> : <Edit className="w-4 h-4"/>}</button></div></div></div>
                    <div className="lg:col-span-3 bg-brand-glass border border-slate-700/50 rounded-2xl flex flex-col min-h-[70vh]">{interactionMode === 'none' ? <div className="flex-grow flex flex-col justify-center items-center p-6 text-center"><h3 className="text-2xl font-bold mb-4 text-white">Start a Conversation</h3><p className="text-slate-400 mb-6">Choose how to interact with your avatar.</p><div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm"><button onClick={handleStartLiveConversation} className="button-primary w-full !py-3" title="Start a live voice conversation with your avatar"><Mic className="w-5 h-5 mr-2" /> Voice Chat</button><button onClick={handleStartTextChat} className="button-secondary w-full !py-3" title="Start a text-based chat with your avatar"><MessageSquare className="w-5 h-5 mr-2" /> Text Chat</button></div><div className="mt-6 w-full max-w-sm"><CustomSelect label="Voice Style" value={voice} onChange={setVoice} options={voices} /></div>{error && <p className="text-red-400 mt-4 text-sm">{error}</p>}</div> : <><div className="p-4 border-b border-slate-700/50 flex justify-between items-center"><h3 className="font-bold text-lg">{interactionMode === 'voice' ? 'Live Conversation' : 'Text Chat'}</h3><button onClick={handleEndConversation} className="button-danger px-3 py-1.5 text-sm" title="End the current conversation"><Phone className="w-4 h-4 mr-2"/> End</button></div><div className="flex-grow p-4 space-y-4 overflow-y-auto">{transcript.map((entry, i) => (<div key={i} className={`flex items-start gap-3 chat-bubble ${entry.source === 'user' ? 'justify-end' : ''}`}>{entry.source === 'avatar' && <div className="w-8 h-8 rounded-full bg-slate-700 flex-shrink-0 mt-1"><img src={`data:image/png;base64,${avatarBase64}`} className="w-full h-full rounded-full"/></div>}<p className={`px-4 py-2 rounded-2xl max-w-md ${entry.source === 'user' ? 'bg-violet text-white rounded-br-none' : 'bg-slate-700 text-slate-200 rounded-bl-none'}`}>{entry.text}</p></div>))}</div>{interactionMode === 'text' && <div className="p-4 border-t border-slate-700/50"><div className="relative"><input type="text" value={textInput} onChange={e => setTextInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSendTextMessage()} placeholder="Type your message..." disabled={isTextLoading} className="form-input pr-12"/><button onClick={handleSendTextMessage} disabled={isTextLoading || !textInput.trim()} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-violet disabled:opacity-50">{isTextLoading ? <Spinner size="sm" /> : <Send className="w-5 h-5 text-white" />}</button></div></div>}</>}</div>
                 </div>
            )}
        </div>
    );
