...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ isOpen, onClose, plan }) => {
  ...
  if (!planDetails) return null; // Or show an error
           ...
           <form onSubmit={handlePayment} className="space-y-4">
               {error && <p className="bg-red-500/20 text-red-300 text-center text-sm p-3 rounded-lg">{error}</p>}
                <div>
                    ...
                    <input type="text" id="card-name" placeholder="John Doe" required className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-brand-purple transition-all"/>
                </div>
                 <div>
                    <label htmlFor="card-number" className="block text-sm font-medium text-gray-300 mb-1">Card Number</label>
                    <div className="relative">
                        <input type="text" id="card-number" placeholder="•••• •••• •••• 4242" required className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 px-3 pl-10 focus:outline-none focus:ring-2 focus:ring-brand-purple transition-all"/>
                ...
                <div className="grid grid-cols-2 gap-4">
                     <div>
                        ...
                        <input type="text" id="card-expiry" placeholder="MM / YY" required className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-brand-purple transition-all"/>
                    </div>
                     <div>
                        <label htmlFor="card-cvc" className="block text-sm font-medium text-gray-300 mb-1">CVC</label>
                        <input type="text" id="card-cvc" placeholder="123" required className="w-full bg-gray-800 border border-gray-600 rounded-lg py-2 px-3 focus:outline-none focus:ring-2 focus:ring-brand-purple transition-all"/>
                    </div>
                </div>
