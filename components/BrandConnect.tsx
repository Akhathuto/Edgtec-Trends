...
const BrandConnect: React.FC<BrandConnectProps> = ({ setActiveTab }) => {
    ...
    const compatibleChannels = user?.channels?.filter(c => ['YouTube', 'TikTok', 'Instagram', 'Facebook', 'Twitch'].includes(c.platform)) || [];
    ...
    const handleFindSponsors = useCallback(async () => {
        ...
        if (!['YouTube', 'TikTok', 'Instagram', 'Facebook', 'Twitch'].includes(selectedChannel.platform)) {
        ...
        setPitchData({});
        try {
            const result = await findSponsorshipOpportunities(selectedChannel.url, selectedChannel.platform as any);
    ...
    const handleGeneratePitch = useCallback(async (opportunity: SponsorshipOpportunity) => {
        ...
        try {
            ...
            const result = await generateBrandPitch(channelName, selectedChannel.platform as any, opportunity.brandName, opportunity.industry);
    ...
    if (!compatibleChannels.length) {
        return (
            <div className="bg-brand-glass border border-slate-700/50 rounded-xl p-8 shadow-xl backdrop-blur-xl text-center flex flex-col items-center animate-slide-in-up">
                ...
                <h2 className="text-2xl font-bold mb-2">Connect a Social Media Channel</h2>
                <p className="text-slate-400 mb-6 max-w-md">To find brand sponsors, please add a YouTube, TikTok, Instagram, Facebook, or Twitch channel URL to your profile first.</p>
