import React from 'react';

// Using a data URL for the SVG is a good way to embed it without extra files.
const logoBase64 = "data:image/svg+xml;base64,";

interface LogoProps extends React.ImgHTMLAttributes<HTMLImageElement> {}

// This can be a Server Component as it has no client-side interactivity.
export const UtrendLogo: React.FC<LogoProps> = (props) => (
  // Using a standard img tag is perfectly fine and performant for SVGs, especially data URLs.
  // Next.js Image component is more for optimizing raster images (JPG, PNG, WebP).
  <img 
    src={logoBase64} 
    alt="utrend Logo" 
    {...props} 
    className={`animate-logo-pulse ${props.className || ''}`}
  />
);
