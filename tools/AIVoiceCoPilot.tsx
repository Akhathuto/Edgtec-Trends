...
import { Star, Mic, StopCircle, Play, Info } from '../components/Icons';
import ErrorDisplay from '../components/ErrorDisplay';
import Spinner from '../components/Spinner';

// Fix: A type for the session object is needed since 'LiveSession' is not exported.
// This derives the 'LiveSession' type from the SDK's connect method.
...
function encode(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < len; i++) {
    // Fix: Use the byte value from the array, not the loop index.
...
function createBlob(data: Float32Array): LiveBlob {
  const l = data.length;
...
const AIVoiceCoPilot: React.FC<AIVoiceCoPilotProps> = ({ onNavigate }) => {
  const { user } = useAuth();
            ...
            callbacks: {
                ...
                onmessage: async (message: LiveServerMessage) => {
                    ...
                    if (serverContent?.userTurn?.parts?.[0]?.text) {
                        turnInput += serverContent.userTurn.parts[0].text;
                        setCurrentInput(turnInput);
                    }
                    if (serverContent?.modelTurn?.parts?.[0]?.text) {
                        turnOutput += serverContent.modelTurn.parts[0].text;
