...
import { ToolId, Agent as AgentType, AgentSettings } from '../types';
import { Send, Sparkles, Trash2, Bot, Zap, Sliders, ChevronsRight, Search, X } from '../components/Icons';
...
export const AIAgents: React.FC<AIAgentsProps> = ({ onNavigate }) => {
                    ...
                    {agent.skills && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {agent.skills.map((skill, idx) => (
                          <span key={idx} className="text-[10px] bg-slate-700/50 text-slate-300 px-1.5 py-0.5 rounded border border-slate-600/50">
                            {skill}
                          </span>
                        ))}
                      </div>
